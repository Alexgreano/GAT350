...
bool Image::Load(const std::string& filename, uint8_t alpha)
{
    ...
    /*width = *((int*)(&header[<use the offset(dec) in the table above where the bitmap width in pixels>]));
    height = *((int*)(&header[<use the offset(dec) in the table above where the bitmap height in pixels>]));*/
